...
cd grvy-$grvy_ver
./configure CXXFLAGS="-std=c++11" --prefix=$GRVY_DIR  --enable-boost-headers-only --with-boost=$BOOST_DIR && make -j ${make_cores} && make install
...
cd mfem-$mfem_ver \
    && unset MFEM_DIR \
    && make phip HIP_ARCH=${hip_arch} PREFIX=$mfem_prefix \
...
echo export hip_arch=$hip_arch >> export_env
